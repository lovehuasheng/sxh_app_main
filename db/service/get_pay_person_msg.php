...
$get_pay_person_msg = function($data){
    ...
    //预定义 
    $matching = $model->getTableDesc($data['id'],'id,pid,user_id,other_id,other_user_id,status,other_money,pay_image,cid,create_time',$table);
    if(empty($matching) || $data['user_id'] != $matching[0]['other_user_id']){
        return returnErr(1,'数据错误');
    }
    $arr = array(7,8,10);
    if(in_array($matching[0]['cid'], $arr)){
        $user = $m_user->getCompanyInfo($matching[0]['user_id'],'business_center_id,company_name,legal_person as name,legal_alipay_account as alipay_account,mobile as phone,legal_bank_name as bank_name,legal_bank_account as bank_account');
        $user[0]['tel_number'] = '';
        if(isset($user[0]['business_center_id']) && $user[0]['business_center_id']>0){
            $reuser = $m_user->getCompanyInfo(array('company_id'=>$user[0]['business_center_id']),'company_name,legal_person as name,mobile');
        ...
        $user = $m_user->getUserOneInfo($matching[0]['user_id'],'name,alipay_account,phone,weixin_account,bank_name,bank_account,tel_number,referee_id','sxh_user_info');
        $user[0]['company_name'] = '';
        $user[0]['referee_company_name'] = '';
    ...
    $user[0]['user_id'] = $matching[0]['user_id'];
